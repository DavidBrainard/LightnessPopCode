...
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'Fig_extPaintShadowEffectDecodeBothDecoding.eps') ' ' fullfile(figureDir,'Figure8A_ExampleDecoding1CVRidge.eps')];
unix(copyCmd);
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'Fig_extPaintShadowEffectDecodeBothInferredMatches.eps') ' ' fullfile(figureDir,'Figure8B_ExamplePaintShadowEffectCVRidge1.eps')];
unix(copyCmd);
...
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'Fig_extPaintShadowEffectDecodeBothDecoding.eps') ' ' fullfile(figureDir,'Figure8C_ExampleDecoding2CVRidge.eps')];
unix(copyCmd);
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'Fig_extPaintShadowEffectDecodeBothInferredMatches.eps') ' ' fullfile(figureDir,'Figure8D_ExamplePaintShadowEffect2CVRidge.eps')];
unix(copyCmd);

% Figure 9: Envelope decoding
analysisOutputFigDir = fullfile(outputBaseDir,'xExtractedPlots','fitrcvridge_cls-mvmaSMO_pca-no_notshf_nopsshf_sykp_ft-gain_2_1','ST140703lightness0001_115px_-15cx_-72cy_g/');
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'Fig_extPaintShadowEffectRMSEEnvelope.eps') ' ' fullfile(figureDir,'Figure9A_SingleSessionEnvelopeCVRidge.eps')];
unix(copyCmd);
...
copyCmd = ['cp ' fullfile(analysisOutputFigDir,'summaryPaintShadowEnvelopeVsRMSE.eps') ' ' fullfile(figureDir,'Figure9B_PaintShadowEffectSummaryCVRidge.eps')];
unix(copyCmd);
